...
> **About this repository** — This project extends the open-source [CC Switch](https://github.com/farion1231/cc-switch) (MIT) with a per-API-Key **usage limit (budget) feature**: money (USD/CNY) or token caps enforced by the local proxy. A prebuilt macOS DMG is available in [Releases](https://github.com/Xhoryon/ccswitch-usage-plugin/releases); see [docs/development_log.md](docs/development_log.md) for the design notes.
...
- **API Key usage limit** — Per-credential money (USD/CNY with a local exchange rate) or token caps enforced by the local proxy: once the limit is reached, new requests are rejected with a clear error while running requests finish; live progress, threshold warnings, and usage reset without touching history
