...
impl RequestForwarder {
    ...
    async fn forward_with_retry_inner(
        ...
        for provider in providers.iter() {
            ...
            // [Budget] 转发前使用限额闸门：在请求发往上游之前检查该 provider
            // credential 的预算状态，达到限额直接拒绝（绝不先消费上游配额）。
            // failover 链上每个 provider 检查各自的 Key；BudgetExhausted 在
            // categorize_proxy_error 中落入 NonRetryable，不会继续消耗其他
            // provider 的尝试名额。OAuth 类 provider 无静态 key，跳过判定。
            ...
            if let Some(ref fingerprint) = credential_fingerprint {
                if let Err(rejection) = self.router.db().check_budget_before_forward(
                    &provider.id,
                    &provider.name,
                    app_type_str,
                    fingerprint,
