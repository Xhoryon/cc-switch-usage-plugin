...
pub enum ProxyError {
    ...
    /// 携带结构化明细（provider/used/limit），错误体 type = usage_limit_reached。
    #[error("{message}")]
    BudgetExhausted {
        message: String,
    ...
    fn into_response(self) -> Response {
        let (status, body) = match &self {
            ...
            ProxyError::BudgetExhausted { message, detail } => {
                ...
                error_obj.insert("type".to_string(), json!("usage_limit_reached"));
            ...
            _ => {
                let (http_status, message) = match &self {
                    ...
                    ProxyError::UpstreamError { .. } | ProxyError::BudgetExhausted { .. } => {
                        unreachable!()
                    }
                };
