...
> **关于本仓库** — 本项目在开源 [CC Switch](https://github.com/farion1231/cc-switch)（MIT）基础上新增了按 API Key 的**使用限额（预算）功能**：金额（USD/CNY）或 Token 上限，由本地代理强制执行。macOS 预构建 DMG 见 [Releases](https://github.com/Xhoryon/ccswitch-usage-plugin/releases)，设计说明见 [docs/development_log.md](docs/development_log.md)。
...
- **API Key 使用限额** — 按凭证设置金额（USD/CNY，本地汇率换算）或 Token 上限，由本地代理强制执行：达到限额后新请求会被拒绝并返回明确错误，进行中的请求正常完成；实时进度、阈值告警与用量重置（不删历史）
