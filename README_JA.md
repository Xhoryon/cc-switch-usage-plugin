...
> **このリポジトリについて** — 本プロジェクトは、オープンソースの [CC Switch](https://github.com/farion1231/cc-switch)（MIT）に API Key ごとの**使用制限（バジェット）機能**を追加したものです：金額（USD/CNY）またはトークン上限をローカルプロキシが強制します。macOS ビルド済み DMG は [Releases](https://github.com/Xhoryon/ccswitch-usage-plugin/releases) を、設計メモは [docs/development_log.md](docs/development_log.md) をご覧ください。
...
- **API Key 使用制限** -- クレデンシャルごとに金額（USD/CNY、ローカル為替レート）またはトークン上限を設定し、ローカルプロキシが強制：上限に達すると新しいリクエストは明確なエラーで拒否され、実行中のリクエストは最後まで完了。リアルタイムの進捗・しきい値警告・履歴を壊さない使用量リセット
