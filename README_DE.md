...
> **Über dieses Repository** — Dieses Projekt erweitert das Open-Source-[CC Switch](https://github.com/farion1231/cc-switch) (MIT) um eine **Nutzungslimit-Funktion (Budget) pro API-Key**: Geld- (USD/CNY) oder Token-Limits, erzwungen durch den lokalen Proxy. Vorgefertigte macOS-DMG unter [Releases](https://github.com/Xhoryon/ccswitch-usage-plugin/releases), Design-Notizen in [docs/development_log.md](docs/development_log.md).
...
- **API-Key-Nutzungslimit** — Geldlimit (USD/CNY mit lokalem Wechselkurs) oder Token-Limit pro Credential, erzwungen durch den lokalen Proxy: Nach Erreichen des Limits werden neue Anfragen mit einer klaren Fehlermeldung abgelehnt, laufende Anfragen laufen fertig; Live-Fortschritt, Schwellenwertwarnungen und Nutzungszurücksetzung ohne Löschen des Verlaufs
