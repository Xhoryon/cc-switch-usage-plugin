...
- **ダイアログ高さ（V1.2.1 導入、V1.2.2 修正）**：明示的な
  `max-h-[calc(100vh-1rem)] overflow-hidden` で共有 90vh 上限を上書きし、
  スクロール領域をその内側で機能——タイトルとフッターは常に見えます。
  **`dvh` ではなく `vh` を使うこと**：dvh は旧 WKWebView（macOS 12/13）で
  解析されず、tailwind-merge が共有 90vh を先に除去するため、制約全体が
  消えていました（V1.2.1 の教訓）。カスタム窗長入力（`min-w-0 flex-1`）と
  時間/日セグメント（`w-28 shrink-0`）は狭いダイアログで溢れません。
