...
> 適用バージョン：**V1.2.0**（公開リリース、2026-09-18。インストーラ修正版で
...
### 4.3 リセット周期と遅延ロールオーバー（V1.0.1 導入、V1.2.0 で意味論を再設計）

**ウィンドウ起点（V1.2.0 以降）**：初回作成、キー交換、**オフ→オン**、**有効状態でのリセット設定変更**のいずれかで `usage_start_at` は「今」に設定されます——オプションを有効にした時点から集計し、有効化前の使用量は決して取り込みません。保存経路の日曆境界への逆整列（V1.0.1 の daily 動作）は削除されました。

`ResetPeriod`：
| 周期 | ロールオーバー | 例（2026-09-18 金 15:27 に有効化） |
| --- | --- | --- |
| never | なし、手動リセットのみ | — |
| hourly | ローカルの整時境界 | 15:00 起算、16:00 にリセット |
| daily | ローカルの零時境界 | 15:27 起算、翌 00:00 にリセット |
| weekly | ローカルの月曜零時（ISO 週） | 翌月曜まで |
| monthly | ローカルの 1 日零時 | 翌月 1 日まで |
| **custom**（V1.2.0） | **アンカー + 窗長の整數倍**（N 時間/日） | 15:27 起算、N 時間/日ごとにロール |

- 日曆周期の有効起点 = `max(usage_start_at, 現在の日曆境界)`（有効化時刻
  から次の境界までは有効化時刻を維持、境界を跨ぐとロール）。
- custom の有効起点 = `アンカー + floor((now-アンカー)/窗長) × 窗長`；
  窗長は `window_length`（1~10000）+ `window_unit`（hours/days）に保存、
  custom のみ携帯可能。
...
2. **status（読み取り経路）**：同じ規則で即時計算し `resetPeriod` /
   `windowLength` / `windowUnit` / `nextResetAt` を返すが**書き戻さない**——
   読み取りは副作用なし。
3. **save（書き込み経路、V1.2.0）**：周期の省略時は `never`。不正値は保存を
   拒否。ウィンドウ起点は上記 4 条件に従います（オフ→オンとリセット設定変更 →
   今）。日曆境界への逆整列は行いません。custom の長さ/単位は厳格に検証
   （1~10000、hours/days）し、custom 以外はウィンドウ欄を携帯できません。
...
- Dialog 構成：オフ時はトグル行のみ（簡潔）。オンで同じ Dialog 内に展開
  ...
  **リセット周期の 6 セグメント選択（3×2、V1.2.0 からカスタム）**→
  カスタム窗長入力 + 時間/日セグメント → enforcement/未知価格の警告 →
  使用量/残額/進捗 → [使用量をリセット] [キャンセル] [保存]。
- 「次のリセット：<ローカル時刻>」は選択中の周期が**保存済み**設定と一致する
  場合のみ表示。切替中に古い境界を表示しない。
- **カード使用量バッジ（V1.2.0）**：メーターアイコンの横に表示トグル
  （Eye/EyeOff、provider ごとに localStorage 保存——UI 設定のみ）；オンで
  `{percent}%`（状態色）+ `⏱{次回リセットまで}` を表示。制限オフまたは
  百分比なしでは自動的に非表示。
- **ホバー説明（V1.2.1）**：カードのアイコン列の全ボタン（目=バッジ
  トグル、メーター=制限設定を含む）はホバーで Radix Tooltip で説明
  （ProviderActions `TipButton`、disabled 時は span 包裹）；WKWebView で
  不安定なネイティブ `title` を置き換え。
- **ダイアログのアンカー配置（V1.2.1/V1.2.2 の 2 回の max-h 修正が失敗した
  後の V1.2.3 最終案）**：DialogContent は**インライン style**
  `position:fixed; top:2.5rem; bottom:0.75rem; left:50%;
  transform:translateX(-50%); maxHeight:none` —— top/bottom のダブルアンカー
  でボックス高は常に「ウィンドウ高 − 52px」となり、物理的にウィンドウを
  はみ出せません。**教訓**：① tailwind-merge は負の任意値をデデュープ
  しない——共有コンポーネントの `translate-y-[-50%]` が上書きクラスと共存
  し、CSS カスケードで勝つため、max-h 修正はすべて相殺された（V1.2.1 の
  dvh 失効と V1.2.2 の vh 無効の共通根因）；② インライン style は優先度が
  最も高く、コンポーネントライブラリの配置を上書きする唯一の確実な手段。
  カスタム窗長入力（`min-w-0 flex-1`）と時間/日セグメント（`w-28 shrink-0`）
  は狭いダイアログで溢れません。
- **ネストされた角丸（V1.2.2）**：スクロール領域に `px-4 py-1` を付け、
  ガラスカード（rounded-xl）を外枠（rounded-lg）から内縮——外 6px ⊃ 内
  12px の半径差が「内は角丸、外は直線」に見える問題を解消。
- **スクロール領域 `space-y-4`（V1.2.3）**：有効化カードと設定カードの
  間隔を広げました（ユーザーフィードバック「近すぎる」）。
...
| **V1.1.1** | 2026-09-18 | **インストーラ修正**：アプリバンドルに完全な ad-hoc 署名（「破損」警告と初回ドラッグ未登録を修正）、DMG から迷子の `.VolumeIcon.icns` を削除、README/リリースノートに初回起動の承認手順を追記。機能は V1.1.0 と同一 |
| **V1.2.0** | 2026-09-19 | ウィンドウ意味論の再設計（オフ→オン/設定変更で今から起算、日曆逆整列を廃止）+ カスタムローリングウィンドウ（N 時間/日、schema v23）+ ダイアログのスクロール/ドラッグ修正 + カード使用量バッジ |
| **V1.2.1** | 2026-09-19 | UI 修正：ダイアログ高さ制約（タイトルが窓外に出ない）、カスタム窗長列の溢れ、カードアイコン列のホバー説明 |
| **V1.2.2** | 2026-09-19 | ダイアログレイアウトの最終修正：max-h を vh に変更（旧 WKWebView で dvh が無効だったのが V1.2.1 無効の根因）+ ガラスカードの内縮でネスト角丸を解消 |
| **V1.2.3** | 2026-09-19 | ダイアログを top/bottom インラインアンカー配置に変更（実証済み根因：tailwind-merge が負の translate-y をデデュープせず max-h 修正を相殺）、有効化/設定カードの間隔を拡大 |
