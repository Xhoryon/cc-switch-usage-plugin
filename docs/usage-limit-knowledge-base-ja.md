...
- **ダイアログ高さ（V1.2.1）**：明示的な
  `max-h-[calc(100dvh-1rem)] overflow-hidden` で共有 90vh 上限を上書き
  （オーバーレイタイトルバーのウィンドウでは 90vh ではタイトルが窓外に
  出る）；スクロール領域がその内側で機能し、カスタム窗長入力
  （`min-w-0 flex-1`）と時間/日セグメント（`w-28 shrink-0`）は狭い
  ダイアログで横に溢れません。
