...
- **悬停提示（V1.2.1）**：卡片图标行全部按钮（含眼睛=徽标开关、仪表盘=
  限额设置）使用 Radix Tooltip 悬停说明（ProviderActions `TipButton`，
  disabled 态外包 span）；替换 WKWebView 下不可靠的原生 `title`。
- **对话框锚定定位（V1.2.1/V1.2.2 两轮 max-h 修复失败后的 V1.2.3 终案）**：
  DialogContent 用**内联 style** `position:fixed; top:2.5rem; bottom:0.75rem;
  left:50%; transform:translateX(-50%); maxHeight:none` —— top/bottom 双锚定
  使盒子高度恒等于「窗口高 − 52px」，数学上不可能超出窗口。**教训**：
  ① twMerge 不去重负值任意值——共享组件的 `translate-y-[-50%]` 与覆盖类
  共存后按 CSS 级联胜出，任何 max-h 修复都被平移抵消（V1.2.1 dvh 失效 +
  V1.2.2 vh 仍无效的共同根因）；② 内联样式优先级最高，是覆盖组件库定位的
  唯一可靠手段。自定义窗口行输入 `min-w-0 flex-1`、小时/天分段
  `w-28 shrink-0`，窄窗不横向裁切。
- **嵌套圆角（V1.2.2）**：滚动容器 `px-4 py-1` 让玻璃卡（rounded-xl）
  与外框（rounded-lg）之间留出内缩——外 6px ⊃ 内 12px 的半径差不再
  产生「内圆角外直线」的观感。
- **滚动容器 `space-y-4`（V1.2.3）**：启用卡与配置卡两块玻璃卡间距拉开
  （用户反馈「挨得太近」）。
...
| **V1.2.1** | 2026-09-19 | 界面细节修复：对话框高度约束（标题不再顶出）、自定义窗口行溢出、卡片图标行悬停提示（含眼睛开关说明） |
| **V1.2.2** | 2026-09-19 | 对话框布局终修：max-h 改用 vh（dvh 在旧 WKWebView 失效是 V1.2.1 无效的根因）+ 玻璃卡内缩解决嵌套圆角 |
| **V1.2.3** | 2026-09-19 | 对话框改 top/bottom 内联锚定定位（twMerge 不去重负值任意值导致平移抵消 max-h 的根因实锤），启用卡与配置卡间距拉开 |
