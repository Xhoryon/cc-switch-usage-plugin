...
- **对话框高度（V1.2.1）**：显式 `max-h-[calc(100dvh-1rem)] overflow-hidden`
  覆盖共享 90vh 上限（Overlay 标题栏窗口下 90vh 会把标题顶出窗口顶缘），
  内容区滚动容器在其内生效；自定义窗口行输入 `min-w-0 flex-1`、
  小时/天分段 `w-28 shrink-0`，窄窗不横向裁切。
