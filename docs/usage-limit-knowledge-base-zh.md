...
- **对话框高度（V1.2.1 引入、V1.2.2 修正）**：显式
  `max-h-[calc(100vh-1rem)] overflow-hidden` 覆盖共享 90vh 上限，内容区
  滚动容器在其内生效——标题/底部按钮恒定可见。**必须用 `vh` 而非 `dvh`**：
  dvh 在旧版 WKWebView（macOS 12/13）不被解析，且 tailwind-merge 会先去重
  掉共享的 90vh，导致整条约束失效（V1.2.1 的教训）。自定义窗口行输入
  `min-w-0 flex-1`、小时/天分段 `w-28 shrink-0`，窄窗不横向裁切。
